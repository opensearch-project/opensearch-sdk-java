...
package org.opensearch.sdk.missingdoclet;
...
public class MissingDoclet extends StandardDoclet {
    ...
    private void checkComment(Element element) {
        ...
        if (tree == null || tree.getFirstSentence().isEmpty()) {
            ...
            var normalized = tree.getFirstSentence().get(0).toString().replace('\u00A0', ' ').trim().toLowerCase(Locale.ROOT);
    ...
    private boolean hasInheritedJavadocs(Element element) {
        boolean hasOverrides = element.getAnnotationMirrors()
            .stream()
        ...
        if (element instanceof ExecutableElement) {
            ...
            Iterable<Element> superTypes = () -> superTypeForInheritDoc(thisMethod.getEnclosingElement()).iterator();
